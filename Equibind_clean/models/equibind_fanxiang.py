...
class IEGMN(nn.Module):
        ...
        for layer_mini_idx in range(1,3):
            self.minigraph.append(
                IEGMN_Layer_mini(orig_h_feats_dim=input_node_feats_dim,
                            ...
                            dropout=dropout,
                            save_trajectories=save_trajectories, **kwargs))
        self.liner_new =nn.Linear(iegmn_lay_hid_dim, input_node_feats_dim)
    ...
    def forward(self, pocket_label, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        for l in pocket_label:
            rec_overlapped_points_idx = torch.where(l == 0)[0]
            rec_no_overlapped_points_idx = torch.where(l == 1)[0]
            h_feats_rec_l=self.transformer(coords_rec[idx: idx + len(l)].unsqueeze(0), h_feats_rec[idx: idx + len(l)].unsqueeze(0), rec_overlapped_points_idx,
