...
class IEGMN_Layer_mini(nn.Module):
    def __init__(
            self,
            orig_h_feats_dim,
            h_feats_dim,  # in dim of h
        ...
        super(IEGMN_Layer_mini, self).__init__()
    ...
    def forward(self, rec_graph, coords_rec, h_feats_rec, original_receptor_node_features, orig_coords_rec, mask):
        with rec_graph.local_scope():
            ...
            cross_attention_rec_feat = cross_attention(self.att_mlp_Q(h_feats_rec),
                                                       self.att_mlp_K(h_feats_rec),
                                                       self.att_mlp_V(h_feats_rec),mask,
                                                       self.cross_msgs)
            if self.rec_evolve:
                rec_graph.update_all(self.update_x_moment_rec, fn.mean('m', 'x_update'))
                x_evolved_rec = self.x_connection_init * orig_coords_rec + (1. - self.x_connection_init) * \
            ...
            return x_evolved_rec, node_upd_receptor
...
class IEGMN(nn.Module):
    ...
    def __init__(self, n_lays, debug, device, use_rec_atoms, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        self.transformer = GeometricTransformer(
            ...
            hidden_dim=68,
            num_heads=4,
            blocks=['self','self','self','self','self','self'],
            sigma_d=0.2,
            sigma_a=15,
            angle_k=3,
            reduction_a='max',
        ...
        # if self.use_mean_node_features:
        #     input_node_feats_dim += 5
        self.minigraph = nn.ModuleList()
        self.minigraph.append(
            IEGMN_Layer_mini(orig_h_feats_dim=input_node_feats_dim,
                        h_feats_dim=input_node_feats_dim,
                        out_feats_dim=iegmn_lay_hid_dim,
                        nonlin=nonlin,
                        cross_msgs=self.cross_msgs,
                        ...
                        dropout=dropout,
                        save_trajectories=save_trajectories, **kwargs))
        for layer_mini_idx in range(1,3):
            self.minigraph.append(
                IEGMN_Layer_mini(orig_h_feats_dim=input_node_feats_dim,
                            h_feats_dim=iegmn_lay_hid_dim,
                            out_feats_dim=iegmn_lay_hid_dim,
                            ...
                            dropout=dropout,
                            save_trajectories=save_trajectories, **kwargs))
        self.liner_new =nn.Linear(68, 69)
    ...
    def forward(self, pocket_label, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        mask_rec = get_mask(rec_graph.batch_num_nodes(), rec_graph.batch_num_nodes(), self.device)



        for i, layer in enumerate(self.minigraph):

            coords_rec, h_feats_rec = layer(rec_graph=rec_graph,
        ...
        h_feats_rec_list = []
        idx = 0
        for l in pocket_label:
            rec_overlapped_points_idx = torch.where(l == 1)[0]
            rec_no_overlapped_points_idx = torch.where(l == 0)[0]
            s = h_feats_rec[idx: idx + len(l)]
            s1 = torch.cat([s,torch.zeros([s.size()[0],4]).to('cuda')],dim=-1)
            for id in rec_overlapped_points_idx:
                s1[id,-4:] =1
            h_feats_rec_l=self.transformer(coords_rec[idx: idx + len(l)].unsqueeze(0), s1.unsqueeze(0), rec_overlapped_points_idx,
             rec_no_overlapped_points_idx)
            h_feats_rec_l=h_feats_rec_l.squeeze(0)
            h_feats_rec_list.append(h_feats_rec_l)
            idx += len(l)
        h_feats_rec=torch.cat(h_feats_rec_list)
        h_feats_rec = self.liner_new(h_feats_rec)
