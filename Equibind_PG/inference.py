...
def inference(args, tune_args=None):
    ...
    # data = PDBBind(pocket_dir='./data/pocket_label_test',device=device, complex_names_path=args.test_names, **args.dataset_params)
    # data = PDBBind(pocket_dir='./data/unseen_label_test',device=device, complex_names_path=args.test_names, **args.dataset_params)
    data = PDBBind(pocket_dir='./data/pocket_label_fabind_toequi/pocket_label_fabind_test',device=device, complex_names_path=args.test_names, **args.dataset_params)
    ...
    # args.checkpoint = '/home/tinama/project/EquiBind/runs/best_checkpoint.pt'
    ...
    for conformer_id in range(args.num_confs):
        ...
        for i, batch in tqdm(enumerate(loader)):
            with torch.no_grad():
                pocket_label,lig_graphs, rec_graphs, ligs_coords, recs_coords, all_rec_coords, pockets_coords_lig, geometry_graph, names, idx = tuple(
                    batch)
                # lig_graphs_orignal = copy.deepcopy(lig_graphs)
                # rec_graphs_orignal = copy.deepcopy(rec_graphs)
                # geometry_graphs_orignal = copy.deepcopy(geometry_graph)
                #
                # pocket_pred = pocket_model(lig_graphs, rec_graphs, geometry_graph.to(device), complex_names=names, epoch=0)
                ...
                ligs_coords_pred, ligs_keypts, recs_keypts, rotations, translations, geom_reg_loss = model(pocket_label, lig_graphs,
                                                                                                           rec_graphs,
                                                                                                           complex_names=names,
                                                                                                           epoch=0,
                ...
                # all_dispo.extend(dispo)
                # all_disnopo.extend(disnopo)
                # all_dislig.extend(dislig)
                # all_featurerec.extend(feature_r)
                # all_featurelig.extend(feature_l)
                # all_pocketlabel.extend(pocket_label)
                # all_reccoords.extend(recs_coords)
        ...
        if args.run_corrections:
            ...
            for i, (prediction, target, lig_keypts, rec_keypts, name) in tqdm(enumerate(
                    zip(results['predictions'], results['targets'], results['lig_keypts'], results['rec_keypts'],
                        results['names']))):
                lig = read_molecule(os.path.join('data/PDBBind/PDBBind_processed/', name, f'{name}_ligand.sdf'), sanitize=True)
                if lig == None:  # read mol2 file if sdf file cannot be sanitized
                    lig = read_molecule(os.path.join('data/PDBBind/PDBBind_processed/', name, f'{name}_ligand.mol2'), sanitize=True)
                ...
                all_coords_optim.append(coords_pred_optimized)
                all_coords_native.append(coords_native)
            ...
            # print('rec_dispo:', np.mean(all_dispo))
            # print('rec_disnopo:', np.mean(all_disnopo))
            # print('lig_dis:', np.mean(all_dislig))
