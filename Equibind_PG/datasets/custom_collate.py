...
def graph_collate_revised(batch):
    pocket_label,lig_graphs, rec_graphs, ligs_coords, recs_coords,all_rec_coords, pockets_coords_lig,geometry_graph, complex_names, idx = map(list, zip(*batch))
    geometry_graph = dgl.batch(geometry_graph) if geometry_graph[0] != None else None
    return pocket_label,dgl.batch(lig_graphs), dgl.batch(rec_graphs), ligs_coords, recs_coords, all_rec_coords, pockets_coords_lig,geometry_graph, complex_names, idx
