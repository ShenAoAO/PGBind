...
class PDBBind(Dataset):
    ...
    def __init__(self, pocket_dir='./data/pocket_label_train',device='cuda:0',
        ...
        # self.processed_dir = 'data/processed/unseen'
    ...
    def __getitem__(self, idx):
        ...
        if self.rec_subgraph:
            ...
            if self.random_rec_atom_subgraph:
                ...
                assert rec_graph.num_nodes() > 0
        pocket_label=torch.load(os.path.join(self.pocket_dir,'{}.npz'.format(self.complex_names[idx])))

        geometry_graph = self.geometry_graphs[idx] if self.geometry_regularization or self.geometry_regularization_ring else None
        if self.lig_structure_graph:
            return pocket_label,lig_graph.to(self.device), rec_graph.to(self.device), self.masks[idx], self.angles[idx], lig_coords, rec_graph.ndata['x'], new_pocket_coords, pocket_coords,geometry_graph, self.complex_names[idx], idx
        else:
            return pocket_label,lig_graph.to(self.device), rec_graph.to(self.device), lig_coords, rec_graph.ndata['x'], new_pocket_coords, pocket_coords, geometry_graph, self.complex_names[idx], idx
