...
# from train_pocket import Model
...
def load_model(args, data_sample, device, **kwargs):
    model = globals()[args.model_type](device=device,
                                           lig_input_edge_feats_dim=data_sample[1].edata['feat'].shape[1],
                                           rec_input_edge_feats_dim=data_sample[2].edata['feat'].shape[1],
...
def train(args, run_dir):
    ...
    train_data = PDBBind(pocket_dir='./data/pocket_label_train',device=device, complex_names_path=args.train_names,lig_predictions_name=args.train_predictions_name, is_train_data=True, **args.dataset_params)
    val_data = PDBBind(pocket_dir='./data/pocket_label_val',device=device, complex_names_path=args.val_names,lig_predictions_name=args.val_predictions_name, **args.dataset_params)
    ...
    # model.load_state_dict(torch.load('/home/tinama/project/EquiBind/runs/batch4_zuhe3_5e-5_xu248/best_checkpoint.pt')['model_state_dict'])
    # pocket_model = Model()
    # pocket_model.load_state_dict(torch.load('/home/tinama/project/EquiBind/trainpocket_model/train_test4_11.2_epoch98.pth')[
    #         'model_state_dict'], strict=False)
    # for param in pocket_model.parameters():
