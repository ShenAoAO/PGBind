...
def get_rec_graph(rec, lig,rec_coords, c_alpha_coords, n_coords, c_coords, use_rec_atoms, rec_radius,
                     surface_graph_cutoff, surface_mesh_cutoff, c_alpha_max_neighbors=None, surface_max_neighbors=None):
    if use_rec_atoms:
        return get_hierarchical_graph(rec, lig,rec_coords, c_alpha_coords, n_coords, c_coords,
                                           c_alpha_cutoff=rec_radius,
                                           surface_mesh_cutoff=surface_mesh_cutoff,
                                           c_alpha_max_neighbors=c_alpha_max_neighbors,
                                           surface_max_neighbors=surface_max_neighbors,
        ...
        return get_calpha_graph(rec, lig,c_alpha_coords, n_coords, c_coords, rec_radius, c_alpha_max_neighbors)
...
def get_hierarchical_graph(rec, lig,rec_coords_list, c_alpha_coords, n_coords, c_coords, c_alpha_cutoff=20,
                           c_alpha_max_neighbors=None,
                           surface_graph_cutoff=10, surface_max_neighbors=None,
                           surface_mesh_cutoff=1.72,cutoff=5.0):
    ...
    a=np.zeros(graph.ndata['x'].shape[0])
    new_coords = np.concatenate(graph.ndata['x'].numpy(), axis=0)
    conf = lig.GetConformer()
    lig_coords = conf.GetPositions()
    lig_rec_distance = spa.distance.cdist(lig_coords, new_coords)
    closest_rec_idx = np.argmin(lig_rec_distance, axis=1)
    satisfies_cufoff_mask = np.where(np.min(lig_rec_distance, axis=1) < cutoff)
    a[closest_rec_idx[satisfies_cufoff_mask]] = 1
    graph.ndata['label']=a
...
def get_calpha_graph(rec, lig,c_alpha_coords, n_coords, c_coords, cutoff=20, max_neighbor=None):
    ...
    a = np.zeros(graph.ndata['x'].shape[0])
    new_coords = graph.ndata['x'].numpy()
    conf = lig.GetConformer()
    lig_coords = conf.GetPositions()
    lig_rec_distance = spa.distance.cdist(lig_coords, new_coords)
    closest_rec_idx = np.argmin(lig_rec_distance, axis=1)
    satisfies_cufoff_mask = np.where(np.min(lig_rec_distance, axis=1) < cutoff)
    a[closest_rec_idx[satisfies_cufoff_mask]] = 1
    graph.ndata['label'] = torch.tensor(a)
