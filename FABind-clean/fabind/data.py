...
from fabind.utils.utils import construct_data_from_graph_gvp_mean
import lmdb
import pickle
import os
...
class FABindDataSet(Dataset):
    ...
    def get(self, idx):
        ...
        data['name'] = name
        # name_new_list =[]
        # # name_list = torch.load('./test_clean_3.npz')
        pocket_dir='/home/tinama/project/FABind/pdbbind2020/dataset/pocket_label_new/{}.npz'.format(name)
        if os.path.exists(pocket_dir):
            data['label'] = torch.load('/home/tinama/project/FABind/pdbbind2020/dataset/pocket_label_new/{}.npz'.format(name))
            if data['label'].size(0)!=protein_node_xyz.size(0):
                data['label'] = torch.zeros([protein_node_xyz.size(0),1])
        else:
            data['label'] = torch.zeros([protein_node_xyz.size(0),1])
        return data
...
def get_data(args, logger, addNoise=None, use_whole_protein=False, compound_coords_init_mode='pocket_center_rdkit', pre="/PDBbind_data/pdbbind2020"):
    if args.data == "0":
        ...
        test_unseen_index = []
        with open('/home/tinama/project/FABind/split_pdb_id/unseen_test_index', 'r', ) as file:
            for line in file:
                a = line.strip()
                query_str = "compound_name == '{}'".format(a)
                test_unseen_i = d.query(query_str).index.values
                test_unseen_index.append(test_unseen_i)
        test_unseen = new_dataset[np.hstack(test_unseen_index)]


    return train, valid, test_unseen