...
parser.add_argument("--compound-coords-init-mode", type=str, default="pocket_center_rdkit",
                    choices=['pocket_center_rdkit', 'pocket_center', 'compound_center', 'perturb_3A', 'perturb_4A',
                             'perturb_5A', 'random'])
...
if args.sample_n > 0:
    ...
    train_loader = DataLoader(train, batch_size=args.batch_size, follow_batch=['x', 'compound_pair'], shuffle=True,
                              pin_memory=False, num_workers=num_workers)
...
valid_loader = DataLoader(valid, batch_size=args.batch_size, follow_batch=['x', 'compound_pair'], shuffle=False,
                          pin_memory=False, num_workers=num_workers)
...
from models.model_pocket import *

device = 'cuda'

model = get_model(args, logger, device)
model.load_state_dict(torch.load('/home/tinama/project/FABind/ckpt/best_model.bin'),strict=False)
...
#
# load_model(model, "/home/tinama/project/FABind/epoch_109/model.safetensors", strict=False)
# params = [
#     {'params': [param for name, param in model.named_parameters() if 'transformer' in name], 'lr': 1e-5},
#     {'params': [param for name, param in model.named_parameters() if 'transformer' not in name], 'lr': 1e-6},
# ]
# if args.optim == "adam":
...
if args.lr_scheduler == "constant":
    scheduler_post = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=1.0,
                                                         total_iters=(args.total_epochs - args.warmup_epochs) * len(
                                                             train_loader), last_epoch=last_epoch)
elif args.lr_scheduler == "poly_decay":
    scheduler_post = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=0.0,
                                                       total_iters=(args.total_epochs - args.warmup_epochs) * len(
                                                           train_loader), last_epoch=last_epoch)
    ...
    scheduler_post = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
                                                                T_max=(args.total_epochs - args.warmup_epochs) * len(
                                                                    train_loader), eta_min=1e-5, last_epoch=last_epoch)
elif args.lr_scheduler == "cosine_decay_restart":
    scheduler_post = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, eta_min=0.0001,
                                                                          last_epoch=last_epoch)
...
model.load_state_dict(torch.load('/home/tinama/project/FABind/results/equi_peal_batch1_rt_nxi/models/epoch_398/pytorch_model.bin'))
for epoch in range(last_epoch + 1, args.total_epochs):
    ...
    #     try:
    #         pocket_cls_pred, pocket_cls, protein_out_mask_whole, p_coords_batched_whole, pred_pocket_center = model(data, train=True)
    ...
    #     pocket_coord_loss = args.pocket_distance_loss_weight * pocket_coord_criterion(pred_pocket_center,
    #                                                                                   data.coords_center)
    #
    #     loss = pocket_cls_loss + pocket_coord_loss
    #     try:
    ...
    #         count += 1
    #         true = pocket_cls.detach()[i][:j]
    #         pred = pocket_cls_pred.detach()[i][:j].sigmoid().round().int()
    #         accuracy_1 = accuracy_score(true.cpu().view(-1).numpy(),
    #                                     pred.detach().cpu().view(-1).numpy())
    #         f1_1 = f1_score(true.cpu().view(-1).numpy(), pred.cpu().view(-1).numpy())
    #         recall_1 = recall_score(true.cpu().view(-1).numpy(), pred.cpu().view(-1).numpy())
    ...
    #     accuracy_total_list.append(accuracy)
    ...
    # metrics = {"samples": count, "skip_samples": skip_count}
    ...
    # metrics.update({"pocket_acc": np.mean(accuracy_total_list)})
    # metrics.update({"pocket_f1": np.mean(f1_total_list)})
    # metrics.update({"pocket_recall": np.mean(recall_total_list)})
    #
    ...
    #         metrics = evaluate_mean_pocket_cls_coord_pocket_pred(args, valid_loader,
    #                                                              accelerator.unwrap_model(model), pocket_cls_criterion,
    ...
    if accelerator.is_main_process:
        metrics = evaluate_mean_pocket_cls_coord_pocket_pred(args, test_loader,
                                                            accelerator.unwrap_model(model),pocket_cls_criterion, pocket_coord_criterion,
                                                            accelerator.device)
