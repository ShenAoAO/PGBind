...
def construct_data_from_graph_gvp_mean(args, protein_node_xyz, protein_seq,
    ...
    if args.local_eval:
        if group == 'test':
            ...
            # mol = Chem.MolFromMolFile('./pdbbind2020/renumber_atom_index_same_as_smiles/6h9v.sdf', sanitize=False)
            # Chem.SanitizeMol(mol)
            # mol = Chem.RemoveHs(mol)
            ...
            rdkit_coords = uniform_random_rotation(a)
...
@torch.no_grad()
def evaluate_mean_pocket_cls_coord_pocket_pred(args, data_loader, model, pocket_cls_criterion, pocket_coord_criterion, device):
    # contain the ground truth for classiifcation(may not all)
    accuracy_total_list = []
    f1_total_list = []
    recall_total_list = []
    ...
    for data in tqdm(data_loader, mininterval=args.tqdm_interval):
        data = data.to(device)
        try:
            pocket_cls_pred, pocket_cls, protein_out_mask_whole, p_coords_batched_whole, pocket_coord_pred_direct= model(data)
        except Exception as e:
            print(e)
            continue
        pocket_cls_loss = args.pocket_cls_loss_weight * pocket_cls_criterion(pocket_cls_pred, pocket_cls.float())
